...
const Header: React.FC<HeaderProps> = ({ language, setLanguage, onLogoClick, onBookmarksClick }) => {
  return (
    <header className="bg-gray-800 shadow-lg p-4 flex justify-between items-center sticky top-0 z-20 border-b border-gray-700">
      <div className="flex items-center gap-3">
        <div onClick={onLogoClick} className="flex items-center gap-1 cursor-pointer">
      ...
      <div className="flex items-center space-x-4">
        ...
        >
          <BookmarkIcon />
          <span className={`hidden sm:inline text-base font-semibold ${language === 'bn' ? 'font-bengali' : ''}`}>
        ...
        <div className="flex items-center space-x-2 bg-gray-700 p-1 rounded-full">
          <button
            ...
            className={`px-3 py-1 text-base font-semibold rounded-full transition-colors duration-300 ${
              language === 'en' ? 'bg-orange-500 text-white' : 'text-gray-300 hover:bg-gray-600'
            }`}
          >
            EN
          </button>
          <button
            ...
            className={`px-3 py-1 text-base font-semibold rounded-full transition-colors duration-300 ${
              language === 'bn' ? 'bg-orange-500 text-white font-bengali' : 'text-gray-300 font-bengali hover:bg-gray-600'
            }`}
          >
            বাং
          </button>
